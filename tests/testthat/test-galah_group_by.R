...
test_that("grouped atlas_counts returns expected output", {
  capture_requests("group_by_counts", {
    counts <- galah_call() |>
      identify("Mammalia") |>
...
test_that("grouped atlas_counts returns expected output when limit != NULL", {
  capture_requests("group_by_with_limit", {
    counts <- galah_call() |>
      identify("Mammalia") |>
      group_by(basisOfRecord) |>
      count() |>
...
test_that("atlas_counts returns all counts if no limit is provided", {
  capture_requests("group_by_no_limit", {
    counts <- galah_call() |>
      group_by(month) |>
      atlas_counts()
  })
...
test_that("grouped atlas_counts for species returns expected output", {
  capture_requests("group_by_with_type_species", {
    counts <- galah_call() |>
      identify("Mammalia") |>
      filter(year == 2020) |>
      group_by(month) |>
...
test_that("group_by works for three groups", {
  capture_requests("group_by_3_groups", {
    counts <- galah_call() |>
      identify("cacatuidae") |>
      filter(year >= 2020) |>
