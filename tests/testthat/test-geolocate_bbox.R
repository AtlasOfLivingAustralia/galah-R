purrr_bbox <- purrr::quietly(geolocate_bbox)
quiet_bbox <- function(...){
  purrr_bbox(...) |>
    purrr::pluck("result")}

test_that("`geolocate_bbox()` returns bbox for sf", {
...
test_that("`geolocate_bbox()` returns bbox for bbox", { # FIXME: not backwards compatible with bbox coords?
  ...
  expect_message(geolocate_bbox(bbox), "Data returned for bounding box:")
  expect_equal(quiet_bbox(bbox)[1], expected_polygon)
  expect_equal(attributes(bbox_galah)$bbox, bbox)
})

test_that("`geolocate_bbox()` returns bbox for tibble", {
  ...
  expected_bbox <- sf::st_bbox(c(xmin = 143, xmax = 148, ymin = -29, ymax = -21), 
                               crs = sf::st_crs("WGS84"))
  expect_message(geolocate_bbox(tibble), "Data returned for bounding box:")
  expect_equal(quiet_bbox(tibble)[1], expected_polygon)
  expect_equal(attributes(tibble_bbox)$bbox, expected_bbox)
})

test_that("`geolocate_bbox()` does not accept incorrect tibbles", {
  ...
  expect_error(geolocate_bbox(tibble_wrong))
  expect_error(geolocate_bbox(tibble_bad_colnames))
  expect_error(geolocate_bbox(tibble_invalid_bbox))
})

test_that("`geolocate_bbox()` uses only first coordinates of tibble with many coordinates", {
...
test_that("`geolocate_bbox()` checks number of inputs, uses first argument", { # FIXME
...
test_that("`geolocate_bbox()` checks inputs", {
  ...
  expect_error(geolocate_bbox(wkt))
  expect_error(geolocate_bbox(number))
  expect_error(geolocate_bbox(c_char))
  expect_error(geolocate_bbox(c_numbers))
})

test_that("`geolocate_bbox()` detects invalid spatial objects", {
  impossible_bbox <- sf::st_bbox(c(xmin = 148000, 
                                   xmax = -29000, 
                                   ymin = -29000, 
                                   ymax = -29000),
                                 crs = sf::st_crs("WGS84"))
  expect_error(geolocate_bbox(impossible_bbox), 
               "Invalid spatial object")
})

test_that("`geolocate_bbox()` converts to multipolygon", {
