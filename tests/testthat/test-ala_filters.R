...
test_that("select_filters builds data quality filters", {
  expect_s3_class(select_filters(data_quality_profile = "ALA"),
                  "data.frame")
  expect_error(select_filters(data_quality_profile = "bad"))
  expect_equal(unique(select_filters(data_quality_profile = "CSDM")$include),
               c(TRUE, FALSE))
})
