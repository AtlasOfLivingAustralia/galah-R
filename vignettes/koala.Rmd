...
koala enables the R community to directly access data hosted by the Atlas of Living Australia (ALA).
...
The koala package must be loaded for each new R session:
...
cols <- select_columns(group = "basic", extra = "country")
filters <- select_filters(list(year = seq(2000, 2020)))
occ <- ala_occurrences(taxon_id = top5_banksia,
...
occ <- ala_occurrences(taxon_id = id, geometry = select_location(area = simple_shp))
```
...
# Bivariate record counts
# Currently the ALA APIs can only breakdown by one type of variable. How much extra effort is to do two?
state_counts <- ala_counts(group_by = "state")
states <- state_counts %>% pull(name)

provider_counts <- bind_rows(lapply(states, function(s) {
  filt <- select_filters(list(state = s))
  counts <- ala_counts(filters = filt, group_by = "data_provider", limit = 10)
  counts$state <- s
  counts
}))

# tidy up columns
provider_counts <- provider_counts %>% rename(data_provider = name) %>%
  relocate(state, data_provider, count)

# plot as a stacked bar chart
ggplot(provider_counts) +
  geom_bar(mapping = aes(x = state, y = count, fill = data_provider),
           position = "stack", stat = "identity") + theme(legend.position = "none")
```
...
filters <- select_filters(list("multimedia" = "Image",
...
Various aspects of the koala package can be customized.

### Caching
koala can cache most results to local files. This means that if the same code is run multiple times, the second and subsequent iterations will be faster. This will also reduce load on the ALA servers.
...
If you change the cache_directory to a permanent location, you may wish to add something like this to your .Rprofile file, so that it happens automatically each time the koala package is loaded:
...
ALA requires that you provide a reason when downloading occurrence data (via the koala `ala_occurrences()` function). The reason is set as "scientific research" by default, but you can change this using `ala_config`. See `ala_reasons()` for valid download reasons.
