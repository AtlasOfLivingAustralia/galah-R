...
The argument that has by far the largest output is 
`show_all(fields)`. Fields are used to filter and narrow queries. The full 
...
##  5 assertionUserId       User ID of the person who has made an assertion about thi… fiel… <NA> 
##  6 assertions            A list of all assertions (user and system supplied) for a… fiel… <NA> 
...
##   id     description                                                                type  link 
##   <chr>  <chr>                                                                      <chr> <chr>
## 1 cl2013 ASGS Australian States and Territories Australian Statistical Geography S… laye… http…
...
Above, we showed an example of using `show_all(fields)`; the equivalent 
microfunction is `show_all_fields()`.
...
```
## # A tibble: 4 × 10
##   search_term scientific_name taxon_concept…¹ rank  match…² kingdom phylum class issues verna…³
##   <chr>       <chr>           <chr>           <chr> <chr>   <chr>   <chr>  <chr> <chr>  <chr>  
## 1 reptilia    REPTILIA        https://biodiv… class exactM… Animal… Chord… Rept… noIss… <NA>   
## 2 aves        AVES            https://biodiv… class exactM… Animal… Chord… Aves  noIss… Birds  
## 3 mammalia    MAMMALIA        https://biodiv… class exactM… Animal… Chord… Mamm… noIss… Mammals
## 4 pisces      PISCES          https://biodiv… <NA>  exactM… Animal… Chord… <NA>  noIss… Fishes 
## # … with abbreviated variable names ¹​taxon_concept_id, ²​match_type, ³​vernacular_name
```
...
## 1 REPTILIA        https://biodiversity.org.au/afd/ta… class taxonI… Animal… Chord… Rept… noIss…
## # … with abbreviated variable name ¹​match_type
```
...
##   id           description                                                          type  link 
##   <chr>        <chr>                                                                <chr> <chr>
## 1 month        "Month of observation, specimen collection date. http://rs.tdwg.org… fiel… <NA> 
## 2 cl23         "Local Government Areas 2012 deprecated LGA Boundaries (superceeded… laye… http…
## 3 eventDate    "The ISO formatted date of observation, specimen collection date. h… fiel… <NA> 
## 4 cl10903      "Tenure of Australia's forests (2013) v2.0 Tenure of Australia's fo… laye… http…
## 5 cl10955      "National Indicative Aggregated Fire Extent Dataset 2019-2020 - v20… laye… http…
...
##  2 "Exclude all records with an assertion that the scientific name provided does not ma… "-ass…
##  3 "Exclude all records with an assertion that the scientific name provided is not stru… "-ass…
##  4 "Exclude all records with an assertion that the name and classification supplied can… "-ass…
##  5 "Exclude all records with an assertion that kingdom provided doesn't match a known k… "-ass…
##  6 "Exclude all records with an assertion that the scientific name provided in the reco… "-ass…
##  7 "Exclude all records with an assertion of the occurence is cultivated or escaped fro… "-est…
##  8 "Exclude all records with an assertion of latitude value provided is zero"            "-dec…
##  9 "Exclude all records with an assertion of longitude value provided is zero"           "-dec…
## 10 "Exclude all records with an assertion of  latitude and longitude have been transpos… "-ass…
```
...
```
## # A tibble: 1 × 17
##   name        acronym uid   phone email pubDe…¹ latit…² longi…³ websi…⁴ alaPu…⁵ dateC…⁶ lastU…⁷
##   <chr>       <chr>   <chr> <chr> <chr> <chr>     <dbl>   <dbl> <chr>   <chr>   <chr>   <chr>  
## 1 Allan Herb… CHR     co214 +64 … Herb… "The A…   -43.6    172. http:/… https:… 2015-0… 2021-1…
## # … with 5 more variables: userLastModified <chr>, active <chr>, numRecords <int>,
## #   numRecordsDigitised <int>, geographicDescription <chr>, and abbreviated variable names
## #   ¹​pubDescription, ²​latitude, ³​longitude, ⁴​websiteUrl, ⁵​alaPublicUrl, ⁶​dateCreated,
## #   ⁷​lastUpdated
```
