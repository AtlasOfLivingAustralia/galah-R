...
## 1 reptilia    REPTILIA        urn:lsid:biodiversity.org.au:afd.ta~ class      exactMatch Animalia Chord~ Rept~ noIss~ <NA>           
## 2 aves        AVES            urn:lsid:biodiversity.org.au:afd.ta~ class      exactMatch Animalia Chord~ Aves  noIss~ Birds          
## 3 mammalia    MAMMALIA        urn:lsid:biodiversity.org.au:afd.ta~ class      exactMatch Animalia Chord~ Mamm~ noIss~ Mammals        
## 4 pisces      PISCES          urn:lsid:biodiversity.org.au:afd.ta~ species i~ exactMatch Animalia Chord~ <NA>  noIss~ fishes
```
...
## 1 REPTILIA        urn:lsid:biodiversity.org.au:afd.taxon:682e1228-5b3c-45ff-833b~ class taxonIdMatch Animalia Chordata Reptil~ noIss~
```
...
## 2 "Exclude all records with an assertion that the scientific name provided does not match any of the n~ "-assertions:TAXON_MATCH_NON~
## 3 "Exclude all records with an assertion that the scientific name provided is not structured as a vali~ "-assertions:INVALID_SCIENTI~
## 4 "Exclude all records with an assertion that the name and classification supplied can't be used to ch~ "-assertions:TAXON_HOMONYM"  
## 5 "Exclude all records with an assertion that kingdom provided doesn't match a known kingdom e.g. Anim~ "-assertions:UNKNOWN_KINGDOM"
## 6 "Exclude all records with an assertion that the scientific name provided in the record does not matc~ "-assertions:TAXON_SCOPE_MIS~
```
