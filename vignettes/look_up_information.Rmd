...
```r
search_all(profiles, "ALA") |> show_values() |> head()
```
...
## 2 "Exclude all records with an assertion that the scientific name provided does not mat… "-ass…
## 3 "Exclude all records with an assertion that the scientific name provided is not struc… "-ass…
## 4 "Exclude all records with an assertion that the name and classification supplied can'… "-ass…
## 5 "Exclude all records with an assertion that kingdom provided doesn't match a known ki… "-ass…
## 6 "Exclude all records with an assertion that the scientific name provided in the recor… "-ass…
```
...
```r
search_all(collections, "herbarium") |> show_values() |> head()
```
