...
```
## Error in select_taxa("Chordata"): `select_taxa` only provides information on Australian taxonomy. To search taxonomy for Austria use `taxize`. See vignette('international_atlases') for more information
```
...
```
## Error in select_taxa("Heleioporus"): `select_taxa` only provides information on Australian taxonomy. To search taxonomy for Austria use `taxize`. See vignette('international_atlases') for more information
```
...
```
## Error in validate_filters(df$variable): The following filters are invalid: stateProvince. Use `search_fields()` to get a list of valid options
```
...
```
## Error in select_taxa("Animalia"): `select_taxa` only provides information on Australian taxonomy. To search taxonomy for Austria use `taxize`. See vignette('international_atlases') for more information
```
...
```
## Error in ala_GET(url, path, params = query): Status code 400returned for url https://biocache-ws.biodiversityatlas.at/occurrence/facets?fq=%28taxonConceptID%3A%22urn%3Alsid%3Abiodiversity.org.au%3Aafd.taxon%3A4647863b-760d-4b59-aaa1-502c8cdf8d3c%22%29%20AND%20-%28taxonConceptID%3A%22urn%3Alsid%3Abiodiversity.org.au%3Aafd.taxon%3A065f1da4-53cd-40b8-a396-80fa5c74dedd%22%29&facets=class&flimit=1<pointer: 0x000001f6f3e7d8f0>
```
...
```
## Error in ala_taxonomy(taxa = select_taxa("Chordata"), down_to = "class"): `search_taxonomy` only provides information on Australian taxonomy. To search taxonomy for Austria use `taxize`. See vignette('international_atlases') for more information
```
...
## 3   ¦--Tunicata                   subphylum
## 4   ¦   ¦--Appendicularia             class
## 5   ¦   ¦--Ascidiacea                 class
## 6   ¦   °--Thaliacea                  class
## 7   °--Vertebrata                 subphylum
## 8       °--Gnathostomata           informal
## 9           ¦--Amphibia               class
## 10          ¦--Aves                   class
## 11          ¦--Mammalia               class
## 12          ¦--Pisces              informal
## 13          ¦   ¦--Actinopterygii     class
## 14          ¦   ¦--Chondrichthyes     class
## 15          ¦   °--Sarcopterygii      class
## 16          °--Reptilia               class
```
...
```
## Error in ala_GET(url, path, params = query): Status code 400returned for url https://biocache-ws.biodiversityatlas.at/occurrence/facets?fq=%28lsid%3Aurn%3Alsid%3Abiodiversity.org.au%3Aafd.taxon%3A298d7998-81d6-49e9-858c-56f8837d09fa%29&flimit=1&facets=speciesID<pointer: 0x000001f6f3e7dbf0>
```
