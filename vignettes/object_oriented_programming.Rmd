...
different set of dplyr verbs: `collapse()`, `compute()` and `collect()`. These 
...
```r
x <- request_data("occurrences") |> # note that occurrences is the default, so we'll skip it next time!
  filter(genus == "Crinia", year == 2020) |>
  group_by(species) |>
  collect()
```
