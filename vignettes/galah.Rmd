...
```r
locations <- galah_locations(query = st_read('act_rect.shp'))
```
...
occ <- ala_occurrences(taxa = select_taxa("Eolophus roseicapilla"),
                       filters = galah_filter(stateProvince = "Australian Capital Territory",
                                                year >= 2010,
                                                profile = "ALA"),
...
species <- ala_species(taxa = select_taxa("Rodentia"),
            filters = galah_filter(stateProvince = "Northern Territory"))
head(species)
```
