...
```{r}
select_taxa(term = "Eolophus", return_children = TRUE, include_counts = TRUE)
```
...
functions are provided. For finding field names, use `find_fields()`, for
finding valid field values, use `find_field_values()`. To view information
about spatial fields, use `find_layers()`.
```{r}
fields <- find_fields()
...
occurrence records with [`ala_occurrences()`](#occurrence-data). However, it is
also possible to download data on species via [`ala_species()`](#species-data),
or media content (largely images) via [`ala_media()`](#media_downloads)
Alternatively, users can assess how many records meet their particular criteria
using [`ala_counts()`](#record-counts).
...
downloaded to `R` by first using `ala_occurrences()` to find records that
contain media, and then passing the resulting `recordID` field to `ala_media()`
to download those media files.
