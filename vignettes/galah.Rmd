...
### Taxon information
`select_taxa()` provides information on one or more taxa. It can take either a
scientific name (default), or a unique taxon identifier. 
The output of `select_taxa()` can be provided to `ala_counts()` and `ala_occurrences()` in the `taxa` argument
to filter by taxa.

#### Scientific name lookup
```{r}
# A single taxa
select_taxa("Dasyurus viverrinus")
```

`select_taxa()` can return record counts for taxa with `include_counts = TRUE`, and the child concepts for taxa with `return_children = TRUE`.
```{r}
banksia <- select_taxa(list(genus = "Banksia"), return_children = TRUE, include_counts = TRUE)
