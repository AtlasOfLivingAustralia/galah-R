...
#' species identifiers.
#' @param filter \code{data.frame}: generated by a call to
#' \code{\link{galah_filter}()}
#' @param location \code{string}: generated by a call to
#' \code{\link{galah_location}()}
#' @param select \code{data.frame}: generated by a call to
...
#' \code{\link{atlas_counts}()} using the same arguments that they intend to pass to
...
#' (i.e., taxa, filter, location, select)}
...
#' occ <- atlas_occurrences(location = galah_location(polygon))
#' }
#' 
#' @export
atlas_occurrences <- function(taxa = NULL, filter = NULL, location = NULL,
  ...
  query <- build_query(taxa, filter, location, select, profile)
