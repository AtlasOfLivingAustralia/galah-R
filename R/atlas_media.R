...
atlas_media_internal <- function(request, 
  ...
  ## if thumbnails requested, change url
  if(type == "thumbnail"){
    all_data$imageUrl <- sub("original$", "thumbnail", all_data$imageUrl)
  }
  # NOTE: This should be conditional on mimeType including "^image"
  ...
  download_ok <- download_media(
    df = all_data[, c("imageUrl", "download_path")], 
    verbose)
