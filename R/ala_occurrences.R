...
#' \code{\link{galah_locations}()}
#' @param fields \code{data.frame}: generated by a call to
#' \code{\link{galah_select}()} 
#' @param columns \code{data.frame}: DEPRECATED - use \code{fields} instead
...
#' data.
#' @param refresh_cache \code{logical}: if set to \code{TRUE} and 
...
#' @return A \code{data.frame} of occurrences, containing columns as specified
#' by \code{\link{galah_select}()}. The \code{data.frame} object has the 
#' following attributes:
#' \itemize{
#' \item{a listing of the user-supplied arguments of the \code{data_request} 
#' (i.e., taxa, filters, locations, fields)}
...
#' occ <- ala_occurrences(filters = galah_filter(year = seq(2010, 2020)))
#'
...
#' occ <- ala_occurrences(locations = galah_locations(polygon))
#' }
#' 
#' @export
ala_occurrences <- function(taxa = NULL, filters = NULL, locations = NULL,
  ...
  query <- build_query(taxa, filters, locations, fields, profile)
  ...
  # Add fields to query
  assertion_fields <- fields[fields$type == "assertions", ]
  query$fields <- build_columns(fields[fields$type != "assertions", ])
  query$qa <- build_assertion_columns(assertion_fields)
  if (mint_doi) {
    query$mintDoi <- "true"
  }
...
no valid field names have been provided. To check whether field names are valid, use `search_fields()`")
    }
  )
  ...
  if (nrow(assertion_fields) > 0) {
    df <- fix_assertion_cols(df, assertion_fields$name)
  }
