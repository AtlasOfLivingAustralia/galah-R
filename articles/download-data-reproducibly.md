...
- download data to a specified location so you can find and re-use it
  later
- request a DOI with your download so you can request it again if needed
  (and cite it correctly!)

These steps are independent, so you can choose whether to enact none,
...
to add `mint_doi = TRUE` to
[`atlas_occurrences()`](https://galah.ala.org.au/R/reference/atlas_.md).
A unique DOI will be assigned to the resulting object once the query is
run.

Please note that as of version 2.2.0, DOI generation and reload is
currently supported for queries to the Atlas of Living Australia and
...
If you need to reload this data locally, you can do that by simply
calling
...
More importantly, though, we now have a persistent link to this data
...
![plot of chunk atlas-support](../reference/figures/doi-web-example.png)
...
If you would like other examples, we use this DOI method to reproducibly
download data throughout the [*Cleaning Biodiversity Data in
R*](https://cleaning-data-r.ala.org.au/data-in-this-book.html) book.
